...
(function (ts) {
    ts.Diagnostics = {
        ...
        Enum_declarations_must_all_be_const_or_non_const: { code: 2469, category: 1, key: "Enum declarations must all be const or non-const." },
        In_const_enum_declarations_member_initializer_must_be_constant_expression: { code: 2470, category: 1, key: "In 'const' enum declarations member initializer must be constant expression." },
        const_enums_can_only_be_used_in_property_or_index_access_expressions_or_the_right_hand_side_of_an_import_declaration_or_export_assignment: { code: 2471, category: 1, key: "'const' enums can only be used in property or index access expressions or the right hand side of an import declaration or export assignment." },
        A_const_enum_member_can_only_be_accessed_using_a_string_literal: { code: 2472, category: 1, key: "A const enum member can only be accessed using a string literal." },
        const_enum_member_initializer_was_evaluated_to_a_non_finite_value: { code: 2473, category: 1, key: "'const' enum member initializer was evaluated to a non-finite value." },
        const_enum_member_initializer_was_evaluated_to_disallowed_value_NaN: { code: 2474, category: 1, key: "'const' enum member initializer was evaluated to disallowed value 'NaN'." },
        Property_0_does_not_exist_on_const_enum_1: { code: 2475, category: 1, key: "Property '{0}' does not exist on 'const' enum '{1}'." },
        let_is_not_allowed_to_be_used_as_a_name_in_let_or_const_declarations: { code: 2476, category: 1, key: "'let' is not allowed to be used as a name in 'let' or 'const' declarations." },
        Cannot_initialize_outer_scoped_variable_0_in_the_same_scope_as_block_scoped_declaration_1: { code: 2477, category: 1, key: "Cannot initialize outer scoped variable '{0}' in the same scope as block scoped declaration '{1}'." },
    ...
    function createTypeChecker(host, produceDiagnostics) {
                ...
                if (symbol.flags & 1) {
                    ...
                    if (localDeclarationSymbol && localDeclarationSymbol !== symbol && localDeclarationSymbol.flags & 2) {
                        if (getDeclarationFlagsFromSymbol(localDeclarationSymbol) & 6144) {
                            var varDeclList = ts.getAncestor(localDeclarationSymbol.valueDeclaration, 189);
                            var container = varDeclList.parent.kind === 171 && varDeclList.parent.parent;
                            var namesShareScope = container && (container.kind === 170 && ts.isAnyFunction(container.parent) || (container.kind === 196 && container.kind === 195) || container.kind === 207);
                            if (!namesShareScope) {
        ...
        function checkVariableLikeDeclaration(node) {
            ...
            if (node.kind !== 126 && node.kind !== 125) {
                ...
                if (node.kind === 188 || node.kind === 146) {
